...
const int MAX_OD_NUM = 100;
int main(int argc, const char * argv[]) {
    for (int i = 0; i < MAX_OD_NUM; i++) {
        Network network = Network(180, 180, 1, 5, 5, 1);
        network.generateODPairs("/Users/fxb/Desktop/误差测试/OD_Cluster_" + std::to_string(i) + ".csv");
        network.generateVariables();
        int iteration_time = 0;
        iteration_time = network.iteration(0.01, 0.01).first; // iteration
        network.calPredictionResult("/Users/fxb/Desktop/误差测试/OD_Cluster_Result_" + std::to_string(i) + ".csv");
    }
    return 0;
}
